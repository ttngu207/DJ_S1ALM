...
classdef Mode100 < dj.Computed
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            mintrials_modeweights=5; %fetch1(ANL.Parameters & 'parameter_name="mintrials_modeweights"','parameter_value');
            ...
            trials_R_correct = [fetchn( rel_correct & 'stimtm_presample="1000"' & 'stimtm_sample="-2.5"'    & 'stimtm_earlydelay="-1.6" or  stimtm_latedelay="-0.8"', 'trial', 'ORDER BY trial')];  tint1{num} = [-0.2 0] ;
            trials_L_correct = [fetchn( rel_correct & 'stimtm_presample="1000"' & 'stimtm_sample="1000"' & 'stimtm_earlydelay="-1.6" or  stimtm_latedelay="-0.8"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.2 0] ;
            trials_R_error = [fetchn( rel_error  & 'stimtm_presample="1000"' & 'stimtm_sample="-2.5"' & 'stimtm_earlydelay="-1.6" or  stimtm_latedelay="-0.8"', 'trial', 'ORDER BY trial')];  tint1{num} = [-0.2 0] ;
            trials_L_error = [fetchn( rel_error & 'stimtm_presample="1000"' & 'stimtm_sample="1000"' & 'stimtm_earlydelay="-1.6" or  stimtm_latedelay="-0.8"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.2 0] ;
            ...
            smallest_set_num = min([numel(trials_L_correct),numel(trials_L_error)]);
            ...
            if smallest_set_num>=5
                for i_subsample = 1:1:50
                    weights_subsample(i_subsample,:)= computeModeWeights_correct_and_error_matched_lefttrials  (psth_t_u_tr, trials_R_correct, trials_L_correct,trials_R_error, trials_L_error, tint1{num}, tint2{num}, psth_t_vector, mintrials_modeweights,smallest_set_num);
                end
