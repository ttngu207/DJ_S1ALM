...
classdef RegressionRotationAverageSignif < dj.Computed
    properties
        keySource = (LAB.BrainArea & 'brain_area="all" or brain_area="vS1" or brain_area="ALM"') *  LAB.Hemisphere * EXP.TrainingType * (EPHYS.CellType & 'cell_type="PYR" or cell_type="FS" or cell_type="all"')   * (ANL.OutcomeType & 'outcome_grouping="all"') * (ANL.FlagBasicTrials & 'flag_use_basic_trials=0')  * (ANL.TongueTuning1DType & 'tuning_param_name="lick_horizoffset" or tuning_param_name="lick_rt_video_onset" or tuning_param_name="lick_peak_x"') * (ANL.LickDirectionType);
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            rel =((ANL.RegressionRotation*ANL.SessionPosition* EXP.SessionTraining*EXP.SessionID) & kk) & (ANL.RegressionDecodingSignificant& key);
