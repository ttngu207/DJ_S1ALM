%{
# Projection of the neural acitivity on a mode (neuron weights vector), using normalized psth
-> EXP.Session
-> EPHYS.CellType
...
classdef RegressionProjTrialGo4 < dj.Computed
    properties
         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') *  (ANL.TongueTuning1DType & 'tuning_param_name="lick_horizoffset" or tuning_param_name="lick_rt_video_onset" or tuning_param_name="lick_peak_x"')  * (ANL.OutcomeType & 'outcome_grouping="all"') *  (ANL.FlagBasicTrials & 'flag_use_basic_trials=0') * ANL.RegressionTime33 * (ANL.LickDirectionType & 'lick_direction="left" or lick_direction="right"')  &  ANL.Video1stLickTrialNormalized
    end
    methods(Access=protected)
        function makeTuples(self, key)
            k=key;
            rel=ANL.RegressionTongueSingleUnitGo3  & k;
            if rel.count==0
                return
            end
            ...
            if numel(unique([PSTH.unit]))>1 %i.e. there are more than one cell
                if ~isempty(PSTH)
                    ...
                    M = Modes;
%                     [key] = fn_RegressionprojectSingleTrial2 (M, PSTH, key);
%                     [key] = fn_RegressionprojectSingleTrial (M, PSTH, key);
%                     [key] = fn_RegressionprojectSingleTrial3 (M, PSTH, key);
                    [key] = fn_RegressionprojectSingleTrial4 (M, PSTH, key);
