...
classdef UnitHierarCluster4 < dj.Computed
    properties
...
%                 keySource = ((EPHYS.CellType) & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (LAB.BrainArea & 'brain_area="vS1"') * (LAB.Hemisphere & 'hemisphere="left"') * (EXP.TrainingType & 'training_type="all"');
%         keySource = ((EPHYS.CellType) & 'cell_type="FS"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (LAB.BrainArea & 'brain_area="vS1"') * (LAB.Hemisphere & 'hemisphere="left"') * (EXP.TrainingType & 'training_type="all"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            [cl_id] = fn_ClusterCells4(PTSH_RLconcat, [1:1:2*numel(time(t_idx))], key);
