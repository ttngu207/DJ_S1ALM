...
classdef ProjTrialNormalizedMedianNormalized10 < dj.Computed
    properties
        ...
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"')   * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') *  (ANL.ModeTypeName & 'mode_type_name="Ramping Orthog.111" or mode_type_name="ChoiceMatched"');
        ...
        function makeTuples(self, key)
            ...
            if numel(unique([PSTH.unit]))>1 %i.e. there are more than one cell
                if ~isempty(PSTH)
                    ...
                    [key] = fn_projectSingleTrial_populateNormalized_median_left_right2(Modes, PSTH, key,Param, trial_num_l, trial_num_r );
                    insert(self,key);
                end
            end
        end
