    ...
    methods(Access=protected)
        function makeTuples(self, key)
          ...
          %% Orthogonolize modes to each other via a Gram–Schmidt process - LateDelay Stimulus Ramping
            ...
            % LateDelay on Selective cells only
            
                      num = 50;
            label{num} = 'LateDelay Selective';
            trials1{num} = [fetchn( rel  & 'stimtm_sample="-2.5"' , 'trial', 'ORDER BY trial')];  tint1{num} = [-0.5 0] ;
            trials2{num} = [fetchn( rel   &  'stimtm_sample="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.5 0] ;
            weights{num} = shuffleModeWeights2_selective(psth_t_u_tr,unit_num, trials1{num}, trials2{num}, tint1{num}, tint2{num}, psth_t_vector, mintrials_modeweights, shuffle_num_for_modeweights, trialfraction_for_modeweights,unit_selective_idx);
            ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );

            
                                       %% Orthogonolize modes to each other via a Gram–Schmidt process - LateDelay Ramping Stimulus
            vectors_set = [weights{50},weights{4},weights{1}]; %order of orthogonalization
            mode_orthogonal = fn_gram_schmidt_process(vectors_set);
          ...
          %% Orthogonolize modes to each other via a Gram–Schmidt process - LateDelay Stimulus Ramping
            vectors_set = [weights{50},weights{1},weights{4}]; %order of orthogonalization
            mode_orthogonal = fn_gram_schmidt_process(vectors_set);
            
            % Stimulus
            num = 53;
            label{num} = 'Stimulus Orthog.22';
            weights{num} = mode_orthogonal(:,2);
            ingestMode (weights{num}, tint1{1}, tint2{1},  key, electrode_group, unit_num, label{num}, num, self );
            
            % Ramping
            num = 54;
            label{num} = 'Ramping Orthog.22';
            weights{num} = mode_orthogonal(:,3);
            ingestMode (weights{num}, tint1{4}, tint2{4},  key, electrode_group, unit_num, label{num}, num, self );
            ...
            trials1{num} = [fetchn( rel  & 'stimtm_presample="1000"' &  'stimtm_sample="-2.5"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint1{num} = [-0.2 0] ;
            trials2{num} = [fetchn( rel  & 'stimtm_presample="1000"' &  'stimtm_sample="1000"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.2 0] ;
            ...
            ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );
             ...
             % LateDelay
            num = 82;
            label{num} = 'Choice Pure';
            trials_R_correct = [fetchn( rel_correct  & 'stimtm_presample="1000"' &  'stimtm_sample="-2.5"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint1{num} = [-0.2 0] ;
            trials_L_correct = [fetchn( rel_correct  & 'stimtm_presample="1000"' &  'stimtm_sample="1000"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.2 0] ;
            trials_R_error = [fetchn( rel_error  & 'stimtm_presample="1000"' &  'stimtm_sample="-2.5"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint1{num} = [-0.2 0] ;
            trials_L_error = [fetchn( rel_error   & 'stimtm_presample="1000"' &  'stimtm_sample="1000"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.2 0] ;
            weights {num}= computeModeWeights_correct_and_error  (psth_t_u_tr, trials_R_correct, trials_L_correct,trials_R_error, trials_L_error, tint1{num}, tint2{num}, psth_t_vector, mintrials_modeweights);
            ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );
