...
classdef Mode4 < dj.Computed
    properties
        %         keySource = EXP.Session & EPHYS.TrialSpikes
        keySource = EXP.Session  & EPHYS.Unit & ANL.IncludeUnit & (EPHYS.UnitCellType  & 'cell_type="Pyr"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            num = 3;
            label{num} = 'LateDelay';
            trials1{num} = [fetchn( rel  & 'stimtm_sample="-2.5"' , 'trial', 'ORDER BY trial')];  tint1{num} = [-1.5 0] ;
            trials2{num} = [fetchn( rel   &  'stimtm_sample="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-1.5 0] ;
            weights{num} = shuffleModeWeights(psth_t_u_tr,unit_num, trials1{num}, trials2{num}, tint1{num}, tint2{num}, psth_t_vector, mintrials_modeweights, shuffle_num_for_modeweights, trialfraction_for_modeweights);
            ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );
            ...
            num = 35;
            label{num} = 'Choice';
            trials_R_correct = [fetchn( rel_correct  & 'stimtm_sample="-2.5"', 'trial', 'ORDER BY trial')];  tint1{num} = [-1.5 0] ;
            trials_L_correct = [fetchn( rel_correct  & 'stimtm_sample="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-1.5 0] ;
            trials_R_error = [fetchn( rel_error  & 'stimtm_sample="-2.5"', 'trial', 'ORDER BY trial')];  tint1{num} = [-1.5 0] ;
            trials_L_error = [fetchn( rel_error  & 'stimtm_sample="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-1.5 0] ;
