...
classdef ProjTrial11FSGood < dj.Computed
    properties
        
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="FS"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (EXP.Outcome & 'outcome="hit" or outcome="miss"') * (ANL.ModeTypeName & 'mode_type_name="Ramping Orthog.111" or mode_type_name="Stimulus Orthog.111" or mode_type_name="ChoiceMatched"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                Modes = fetch ( (ANL.Mode11FS* EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition)  & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*');
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition *  ANL.PSTHTrial * EXP.BehaviorTrial) & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*');
            else
                if contains(k.unit_quality,'all')
                    k = rmfield(k,'unit_quality');
                end
                Modes = fetch (ANL.Mode11FS*EPHYS.UnitCellType * EPHYS.UnitPosition & ANL.IncludeUnit2 & k, '*');
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition *  ANL.PSTHTrial * EXP.BehaviorTrial) & ANL.IncludeUnit2 & k, '*');
            end
