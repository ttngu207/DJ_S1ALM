...
classdef RegressionProjTrialAllLicks < dj.Computed
    properties
         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') *  (ANL.TongueTuning1DType & 'tuning_param_name="lick_horizoffset" or tuning_param_name="lick_rt_video_onset" or tuning_param_name="lick_peak_x"')  * (ANL.OutcomeType & 'outcome_grouping="all"') *  (ANL.FlagBasicTrials & 'flag_use_basic_trials=0')  * (ANL.LickDirectionType)  &  ANL.Video1stLickTrialNormalized
    end
    methods(Access=protected)
        function makeTuples(self, key)
            k=key;
            rel=ANL.RegressionTongueSingleUnitAllLicks  & k;
            if rel.count==0
                return
            end
            ...
            rel_PSTH= (EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHTrial * EXP.BehaviorTrial) & ANL.IncludeUnit  & 'early_lick="no early"';
            if contains(k.unit_quality,'ok or good')
                k = rmfield(k,'unit_quality');
                rel_PSTH= rel_PSTH & k & 'unit_quality="ok" or unit_quality="good"' ; 
                PSTH = fetch (rel_PSTH, '*');
            else
             ...
             rel_video=(ANL.VideoNthLickTrial &  k & rel_PSTH );
            ...
            if numel(unique([PSTH.unit]))>1 %i.e. there are more than one cell
                if ~isempty(PSTH)
                    ...
                    M = Modes;
                    [key] = fn_RegressionprojectSingleTrialAllLicks (M, PSTH, key, TONGUE);
                    insert(self,key);
                end
            end
        end
