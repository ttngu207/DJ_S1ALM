...
classdef UnitISI < dj.Computed
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            isi_hist_bin_centers = [-0.02, isi_hist_edges(2:end) + mean(diff(isi_hist_edges(2:end)))./2];
            [isi_hist, ~] = histc(dt_log, isi_hist_edges);

            key.unit_isi = dt;
            key.unit_isi_hist = isi_hist;
            key.unit_isi_hist_bins = isi_hist_bin_centers;
            insert(self, key);
            
%             hold on;
%             b = bar(isi_hist_bin_centers, isi_hist);
