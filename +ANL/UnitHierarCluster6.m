...
classdef UnitHierarCluster6 < dj.Computed
    properties

%         keySource = ((EPHYS.CellType ) & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (LAB.BrainArea & 'brain_area="ALM"') * (LAB.Hemisphere & 'hemisphere="both"') * (EXP.TrainingType & 'training_type="all"') ;
%         keySource = ((EPHYS.CellType) & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (LAB.BrainArea & 'brain_area="vS1"') * (LAB.Hemisphere & 'hemisphere="left"') * (EXP.TrainingType & 'training_type="all"');
%         keySource = ((EPHYS.CellType ) & 'cell_type="FS"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (LAB.BrainArea & 'brain_area="ALM"') * (LAB.Hemisphere & 'hemisphere="both"') * (EXP.TrainingType & 'training_type="all"') ;
        keySource = ((EPHYS.CellType) & 'cell_type="FS"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (LAB.BrainArea & 'brain_area="vS1"') * (LAB.Hemisphere & 'hemisphere="left"') * (EXP.TrainingType & 'training_type="all"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                %fetching data
                Units = fetch ((EPHYS.Unit * EPHYS.UnitPosition * EXP.SessionTraining * EPHYS.UnitCellType) & k & ANL.IncludeUnit2 & 'unit_quality!="multi"', 'ORDER BY unit_uid');
                PSTH_L = struct2table( fetch((ANL.PSTHAverage * EPHYS.Unit * EPHYS.UnitPosition * EPHYS.UnitCellType * EXP.SessionTraining) & ANL.IncludeUnit2 &  k & 'unit_quality!="multi"' & 'outcome="hit"' & 'trial_type_name="l"', '*', 'ORDER BY unit_uid'));
                PSTH_R = struct2table(fetch ((ANL.PSTHAverage * EPHYS.Unit * EPHYS.UnitPosition * EPHYS.UnitCellType * EXP.SessionTraining) & ANL.IncludeUnit2 &  k & 'unit_quality!="multi"' & 'outcome="hit"' & 'trial_type_name="r"', '*', 'ORDER BY unit_uid'));
            else
                if contains(k.unit_quality,'all')
                    k = rmfield(k,'unit_quality');
                end
                %fetching data
                Units = fetch ((EPHYS.Unit * EPHYS.UnitPosition * EXP.SessionTraining * EPHYS.UnitCellType) & k & ANL.IncludeUnit2, 'ORDER BY unit_uid');
                PSTH_L = struct2table( fetch((ANL.PSTHAverage * EPHYS.Unit * EPHYS.UnitPosition * EPHYS.UnitCellType * EXP.SessionTraining) & ANL.IncludeUnit2 &  k & 'outcome="hit"' & 'trial_type_name="l"', '*', 'ORDER BY unit_uid'));
                PSTH_R = struct2table(fetch ((ANL.PSTHAverage * EPHYS.Unit * EPHYS.UnitPosition * EPHYS.UnitCellType * EXP.SessionTraining) & ANL.IncludeUnit2 &  k & 'outcome="hit"' & 'trial_type_name="r"', '*', 'ORDER BY unit_uid'));
            end
            ...
            [cl_id] = fn_ClusterCells5(PTSH_RLconcat, [1:1:2*numel(time(t_idx))], key);
