...
classdef ProjTrialNormalizedMedianNormalizedDA11 < dj.Computed
    properties
        ...
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"')   * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') *  (ANL.ModeTypeName & 'mode_type_name="ChoiceMatched"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHDATrial * EXP.BehaviorTrial) & ANL.IncludeUnit2 & k & 'outcome="hit" or outcome="miss"' & 'unit_quality="ok" or unit_quality="good"' & 'early_lick="no early"', '*');
            else
                if contains(k.unit_quality,'all')
                    k = rmfield(k,'unit_quality');
                end
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHDATrial * EXP.BehaviorTrial) & ANL.IncludeUnit2 & k & 'outcome="hit" or outcome="miss"' & 'early_lick="no early"', '*');
            end
            ...
            if numel(unique([PSTH.unit]))>1 %i.e. there are more than one cell
                if ~isempty(PSTH)
                    ...
                    [key] = fn_projectSingleTrial_populateNormalized_median_left_right4(Modes, PSTH, key,Param, trial_num_l, trial_num_r );
                    insert(self,key);
                end
            end
        end
