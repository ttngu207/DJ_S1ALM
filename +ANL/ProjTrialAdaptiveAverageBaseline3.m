...
classdef ProjTrialAdaptiveAverageBaseline3 < dj.Computed
    properties
        ...
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * EXP.Outcome * ANL.ModeWeightsSign * ANL.ModeTypeName4;
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                Modes = fetch ( (ANL.Mode3 * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition)  & ANL.IncludeUnit & k & 'unit_quality="ok" or unit_quality="good"', '*');
                ...
                Modes = fetch (ANL.Mode3*EPHYS.UnitCellType * EPHYS.UnitPosition & ANL.IncludeUnit & k, '*');
            ...
            if ~isempty(mode_names)
                for imod = 1:1:numel(mode_names)
                    ...
                    Proj =  fetch(ANL.ProjTrialAdaptiveAverage3 & key,'*');
                     if numel(Proj)==0
                         return
                     end
                    kkl.trial_type_name='l';
                    l_proj = fetch1(ANL.ProjTrialAdaptiveAverage3 & key & kkl,'proj_average');
