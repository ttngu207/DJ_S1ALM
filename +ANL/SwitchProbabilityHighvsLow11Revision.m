...
classdef SwitchProbabilityHighvsLow11Revision < dj.Computed
    properties
        keySource = (EXP.Session & (EXP.SessionTask  &'task="s1 stim"')  & (EPHYS.Unit))*ANL.SessionPosition * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType  & 'unit_quality="all" or unit_quality="ok or good"') * (ANL.ModeTypeName & 'mode_type_name="Ramping Orthog.111"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') * (EXP.TrialNameType &  'trial_type_name="l" or trial_type_name="l_-1.6Full" or trial_type_name="l_-1.6Mini"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            rel_Proj = ( ANL.ProjTrial11Revision * EXP.Session * EXP.SessionID  * ANL.SVMdecoderFindErrorTrials ) & key & 'outcome="hit" or outcome="miss"' & (EXP.BehaviorTrial & 'early_lick="no early"') & sprintf('num_units_projected>=%d', min_num_units_projected); 
