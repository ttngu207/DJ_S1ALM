...
classdef ProjVariance2 < dj.Computed
    properties
%         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (EXP.Outcome & 'outcome="response"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') * (ANL.ModeTypeName & ANL.Mode10);
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (EXP.Outcome & 'outcome="response"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') * (ANL.ModeTypeName & 'mode_type_name="ChoiceMatched" or mode_type_name="Stimulus Orthog.111" or mode_type_name="Ramping Orthog.111"');
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality, 'ok or good')
                rel_Proj =(ANL.ProjTrial100*EXP.BehaviorTrial * ANL.TrialTypeGraphic) & k & 'outcome!="ignore"' & 'early_lick="no early"';
                k = rmfield(k,'unit_quality');
                rel_PSTH = (( ANL.PSTHTrial*EXP.BehaviorTrial * EPHYS.Unit * EPHYS.UnitPosition * EPHYS.UnitCellType * ANL.TrialTypeGraphic ) ) & ANL.IncludeUnit2 & k & 'unit_quality!="multi"' & 'outcome!="ignore"' & 'early_lick="no early"';
            elseif contains(k.unit_quality, 'all')
                rel_Proj =( ANL.ProjTrial100 * ANL.TrialTypeGraphic*EXP.BehaviorTrial) & k & 'outcome!="ignore"' & 'early_lick="no early"';
                k = rmfield(k,'unit_quality');
                rel_PSTH = (( ANL.PSTHTrial*EXP.BehaviorTrial * EPHYS.Unit * EPHYS.UnitPosition * EPHYS.UnitCellType * ANL.TrialTypeGraphic ) ) & ANL.IncludeUnit2 & k & 'outcome!="ignore"' & 'early_lick="no early"';
            end
            ...
            trial_var_explained = fn_compute_varaince_DJ2 (rel_Proj,rel_PSTH , time_bins, kkk);
