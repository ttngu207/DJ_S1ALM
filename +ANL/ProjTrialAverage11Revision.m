...
classdef ProjTrialAverage11Revision < dj.Computed
    properties
...
%         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all" or unit_quality="ok or good"') * EXP.Outcome * (ANL.ModeWeightsSign) * ANL.ModeTypeName;
                keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all" or unit_quality="ok or good"') * EXP.Outcome * (ANL.ModeWeightsSign) * (ANL.ModeTypeName & ANL.Mode11Revision );
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                Modes = fetch ( (ANL.Mode11Revision * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition)  & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*');
                ...
                Modes = fetch (ANL.Mode11Revision*EPHYS.UnitCellType * EPHYS.UnitPosition & ANL.IncludeUnit2 & k, '*');
