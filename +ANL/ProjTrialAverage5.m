...
classdef ProjTrialAverage5 < dj.Computed
    properties
...
%                 keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all" or unit_quality="ok or good"') * EXP.Outcome * (ANL.ModeWeightsSign) * (ANL.ModeTypeName & ANL.Mode777 );
                keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all" or unit_quality="ok or good"') * EXP.Outcome * (ANL.ModeWeightsSign) * (ANL.ModeTypeName & ANL.Mode777 );
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                Modes = fetch ( (ANL.Mode777 * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition)  & ANL.IncludeUnit4 & k & 'unit_quality="ok" or unit_quality="good"', '*');
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHAverage) & ANL.IncludeUnit4 & k & 'unit_quality="ok" or unit_quality="good"', '*');
            else
                if contains(k.unit_quality,'all')
                    k = rmfield(k,'unit_quality');
                end
                Modes = fetch (ANL.Mode777*EPHYS.UnitCellType * EPHYS.UnitPosition & ANL.IncludeUnit4 & k, '*');
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHAverage) & ANL.IncludeUnit4 & k, '*');
            end
