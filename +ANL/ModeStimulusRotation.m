...
-> EPHYS.Unit
-> ANL.ModeTypeNameStimulusRotation
...
classdef ModeStimulusRotation < dj.Computed
    properties
...
%             % Stimulus - Sample only
%             rel = (MISC.S1TrialTypeName * ANL.TrialTypeStimTime * EXP.BehaviorTrial) & key & 'early_lick="no early"' & 'outcome!="ignore"' ;
%             num = 1;
%             label{num} = 'Stimulus Sample';
...
%             ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );
            ...
            % Stimulus - EarlyDelay only
            rel = (MISC.S1TrialTypeName * ANL.TrialTypeStimTime * EXP.BehaviorTrial) & key & 'early_lick="no early"' & 'outcome!="ignore"' ;
            num = 11;
            label{num} = 'Stimulus EarlyDelay';
            trials1{num} = [fetchn( rel &   'stimtm_earlydelay="-1.6"', 'trial', 'ORDER BY trial')];  tint1{num} = [-1.6 -1.2];
            trials2{num} = [fetchn( rel &  'stimtm_sample="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-1.6 -1.2];
%             trials1{num} = [fetchn( rel  & 'stimtm_presample="1000"' &  'stimtm_sample="-2.5"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint1{num} = [-2.5 -2];
%             trials2{num} = [fetchn( rel  & 'stimtm_presample="1000"' &  'stimtm_sample="1000"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint2{num} =  [-2.5 -2];
            weights{num} = shuffleModeWeights2(psth_t_u_tr,unit_num, trials1{num}, trials2{num}, tint1{num}, tint2{num}, psth_t_vector, mintrials_modeweights, shuffle_num_for_modeweights, trialfraction_for_modeweights);
            ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );
            ...
            rel = (MISC.S1TrialTypeName * ANL.TrialTypeStimTime * EXP.BehaviorTrial) & key & 'early_lick="no early"' & 'outcome!="ignore"' ;
            num = 12;
            label{num} = 'Stimulus LateDelay';
            trials1{num} = [fetchn( rel &   'stimtm_latedelay="-0.8"', 'trial', 'ORDER BY trial')];  tint1{num} = [-0.8 -0.4];
            trials2{num} = [fetchn( rel &  'stimtm_sample="1000"', 'trial', 'ORDER BY trial')];  tint2{num} = [-0.8 -0.4];
%             trials1{num} = [fetchn( rel  & 'stimtm_presample="1000"' &  'stimtm_sample="-2.5"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint1{num} = [-2.5 -2];
%             trials2{num} = [fetchn( rel  & 'stimtm_presample="1000"' &  'stimtm_sample="1000"' & 'stimtm_earlydelay="1000"' & 'stimtm_latedelay="1000"', 'trial', 'ORDER BY trial')];  tint2{num} =  [-2.5 -2];
            weights{num} = shuffleModeWeights2(psth_t_u_tr,unit_num, trials1{num}, trials2{num}, tint1{num}, tint2{num}, psth_t_vector, mintrials_modeweights, shuffle_num_for_modeweights, trialfraction_for_modeweights);
            ingestMode (weights{num}, tint1{num}, tint2{num},  key, electrode_group, unit_num, label{num}, num, self );
                ...
                %% Orthogonolize modes to each other via a Gram–Schmidt process - LateDelay Ramping Stimulus
            vectors_set = [weights{36},weights{4},weights{11}]; %order of orthogonalization
            mode_orthogonal = fn_gram_schmidt_process(vectors_set);
                     
            % Stimulus
            num = 21;
            label{num} = 'Stimulus EarlyDelay Orthog.111';
            weights{num} = mode_orthogonal(:,3);
            ingestMode (weights{num}, tint1{1}, tint2{1},  key, electrode_group, unit_num, label{num}, num, self );


             %% Orthogonolize modes to each other via a Gram–Schmidt process - LateDelay Ramping Stimulus
            vectors_set = [weights{36},weights{4},weights{12}]; %order of orthogonalization
            mode_orthogonal = fn_gram_schmidt_process(vectors_set);
                     
            % Stimulus
            num = 22;
            label{num} = 'Stimulus LateDelay Orthog.111';
            weights{num} = mode_orthogonal(:,3);
            ingestMode (weights{num}, tint1{1}, tint2{1},  key, electrode_group, unit_num, label{num}, num, self );
