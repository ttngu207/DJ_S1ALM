...
classdef RegressionProjTrialGo < dj.Computed
    properties
         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="ok or good"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') *  (ANL.TongueTuning1DType & 'tuning_param_name="lick_horizoffset" or tuning_param_name="lick_rt_video_onset" or tuning_param_name="lick_peak_x"')  * (ANL.OutcomeType & 'outcome_grouping="all"') *  (ANL.FlagBasicTrials & 'flag_use_basic_trials=0') * ANL.RegressionTime4 * (ANL.LickDirectionType)  &  ANL.Video1stLickTrialNormalized
    end
    methods(Access=protected)
        function makeTuples(self, key)
            k=key;
            rel=ANL.RegressionTongueSingleUnitGo  & k;
            if rel.count==0
                return
            end
