...
classdef ProjVariance3 < dj.Computed
    properties
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (EXP.Outcome & 'outcome="response"  or outcome="hit"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"') * (ANL.ModeTypeName & ANL.Mode10);
        ...
        function makeTuples(self, key)
            ...
            if contains(key.outcome,'response')
                ...
                if contains(k.unit_quality, 'ok or good')
                    rel_Proj =(ANL.ProjTrialDA100*EXP.BehaviorTrial * ANL.TrialTypeGraphic) & k & 'outcome!="ignore"' & 'early_lick="no early"';
                ...
                elseif contains(k.unit_quality, 'all')
                    rel_Proj =( ANL.ProjTrialDA100 * ANL.TrialTypeGraphic*EXP.BehaviorTrial) & k & 'outcome!="ignore"' & 'early_lick="no early"';
                    k = rmfield(k,'unit_quality');
            ...
            elseif contains(key.outcome,'hit')
                if contains(k.unit_quality, 'ok or good')
                    rel_Proj =(ANL.ProjTrialDA100*EXP.BehaviorTrial * ANL.TrialTypeGraphic) & k  & 'early_lick="no early"';
                    k = rmfield(k,'unit_quality');
                ...
                elseif contains(k.unit_quality, 'all')
                    rel_Proj =( ANL.ProjTrialDA100 * ANL.TrialTypeGraphic*EXP.BehaviorTrial) & k &  'early_lick="no early"';
                    k = rmfield(k,'unit_quality');
            ...
            trial_var_explained = fn_compute_varaince_DJ3 (rel_Proj,rel_PSTH , time_bins, kkk);
