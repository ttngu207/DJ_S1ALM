...
classdef ProjTrial100 < dj.Computed
    properties
...
%         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (EXP.Outcome & 'outcome="hit" or outcome="miss"') * (ANL.ModeTypeName & 'mode_type_name="Ramping Orthog.111" or mode_type_name="Stimulus Orthog.111" or mode_type_name="ChoiceMatched"') * (ANL.ModeWeightsSign & 'mode_weights_sign="all"');
        keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * (EXP.Outcome & 'outcome="hit" or outcome="miss"') * (ANL.ModeTypeName & ANL.Mode10) * (ANL.ModeWeightsSign & 'mode_weights_sign="all"');
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                Modes = fetch ( (ANL.Mode10 * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition)  & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*');
                ...
                Modes = fetch (ANL.Mode10*EPHYS.UnitCellType * EPHYS.UnitPosition & ANL.IncludeUnit2 & k, '*');
