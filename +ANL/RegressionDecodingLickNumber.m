...
classdef RegressionDecodingLickNumber < dj.Computed
    properties
        
        keySource = ((EXP.Session  & EPHYS.Unit & ANL.Video1stLickTrial) *ANL.LickDirectionType *  ANL.FlagBasicTrialsDecoding* ANL.OutcomeTypeDecoding* (ANL.TongueTuning1DType & 'tuning_param_name="lick_horizoffset" or tuning_param_name="lick_rt_video_onset" or tuning_param_name="lick_peak_x"') & ANL.IncludeSession -ANL.ExcludeSession);
    end
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            rel_behav= EXP.TrialID & ((EXP.BehaviorTrial * EXP.TrialName  * ANL.TrialTypeGraphic* ANL.VideoLickNumberTrial)   & k  & 'early_lick="no early"' ) & ANL.RegressionProjTrialGo & ANL.VideoTongueValidRTTrial ;
            if rel_behav.count==0
                return
            end
            
            rel_all_lick= (ANL.VideoLickNumberTrial & key ) & rel_behav;
...
%                 temp_rel_lick.count
                ...
                fn_regression_decoding_lick_number (key,self, rel_Nth_lick)
            end
        end
    end
end