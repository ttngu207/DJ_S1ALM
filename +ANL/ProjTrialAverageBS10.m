...
classdef ProjTrialAverageBS10 < dj.Computed
    properties
...
%         keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all" or unit_quality="ok or good"') * EXP.Outcome * (ANL.ModeWeightsSign) * ANL.ModeTypeName;
                keySource = (EXP.Session  & EPHYS.Unit) * (EPHYS.CellType & 'cell_type="Pyr"') * (EPHYS.UnitQualityType & 'unit_quality="all"') * EXP.Outcome * (ANL.ModeWeightsSign & 'mode_weights_sign="all" or mode_weights_sign="negative"' ) * (ANL.ModeTypeName & ANL.Mode10 );
    ...
    methods(Access=protected)
        function makeTuples(self, key)
            ...
            if contains(k.unit_quality,'ok or good')
                ...
                Modes = fetch ( (ANL.Mode10 * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition)  & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*', 'ORDER BY unit');
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHAverage) & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*', 'ORDER BY unit');
                meanFR = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.UnitFiringRate) & ANL.IncludeUnit2 & k & 'unit_quality="ok" or unit_quality="good"', '*', 'ORDER BY unit');

            else
                if contains(k.unit_quality,'all')
                    k = rmfield(k,'unit_quality');
                end
                Modes = fetch (ANL.Mode10*EPHYS.UnitCellType * EPHYS.UnitPosition & ANL.IncludeUnit2 & k, '*', 'ORDER BY unit');
                PSTH = fetch ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.PSTHAverage) & ANL.IncludeUnit2 & k, '*', 'ORDER BY unit');
                meanFR = fetchn ((EXP.SessionID * EPHYS.Unit * EPHYS.UnitCellType * EPHYS.UnitPosition * ANL.UnitFiringRate) & ANL.IncludeUnit2 & k, 'mean_fr_presample', 'ORDER BY unit');
...
%                     end
                    ...
                    for imod = 1:1:numel(mode_names)
                        ...
                        [key, counter] = fn_projectTrialAvg_populate_baseline_subtract(M, PSTH, key, counter,Param, meanFR);
                    end
                    insert(self,key);
                end
            end
