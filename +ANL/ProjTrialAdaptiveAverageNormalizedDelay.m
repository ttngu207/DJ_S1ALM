...
classdef ProjTrialAdaptiveAverageNormalizedDelay < dj.Computed
    properties
        ...
        function makeTuples(self, key)
            ...
            if ~isempty(mode_names)
                for imod = 1:1:numel(mode_names)
                    ...
                    Proj =  fetch(ANL.ProjTrialAdaptiveAverageBaseline & key,'*');
                    ...
                    key.outcome='hit';
                    rel =ANL.ProjTrialAdaptiveAverageBaseline & key;
                    if rel.count==0
                        return
                    end
...
%                     selectivity = movmean(selectivity,[smooth_bins 0], 2, 'omitnan','Endpoints','shrink');
                    max_selectivity = nanmax(selectivity (time>=-0.5 & time<0));
