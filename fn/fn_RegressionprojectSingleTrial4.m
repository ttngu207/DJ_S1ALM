function  [key] = fn_RegressionprojectSingleTrial4(M, PSTH, key)
...
for itr= 1:1:numel(trials)
    ...
    weights = [Mtrial.regression_coeff_b2].*sinh(5*[Mtrial.regression_rsq]); %weights are regression coefficients * regression R^2
    weights=weights';
    %     subplot(2,2,1)
...
%         proj_min_tr(itr)=min(p_tr_smooth(idx_time_to_normalize));
        proj_max_tr(itr)=max(p_tr_smooth);
        proj_min_tr(itr)=min(p_tr_smooth);
