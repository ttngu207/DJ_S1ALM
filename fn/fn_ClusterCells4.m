function [cl_id]=fn_ClusterCells4(PTSH_RLconcat, time, key)
...
dir_save_figure = [dir_root 'Results\Clustering\metrics\'];
if contains(key.brain_area,'vS1')
    n_clust =10; %for vS1 
elseif  contains(key.cell_type,'FS')
    n_clust =30;
else
    n_clust =150; %150 for PYR in ALM;
end
...
corr_thresh_for_merging = 0.75; %won't merge clusters that has correlation value below that


%% Hierarchical cluster analysis.
% Get distances.
d = pdist(PTSH_RLconcat,'correlation');
d_sq = squareform(d);
